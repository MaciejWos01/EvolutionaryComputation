...
int main(){
    ...
    dataToDistance(data);
    int randomLocalOptima[1000][100];
    int bestSolution[100];
    int bestSolutionValue = 100000000;
    float result[1000][5];
    for (int i = 0; i < 1000; i++) {
        struct statistics localStat1 = local1(1);
        result[i][4] = localStat1.bestSolutionValue;
        for (int j = 0; j < 100; j++) {
            randomLocalOptima[i][j] = localStat1.bestSolution[j];
        }
        if (localStat1.bestSolutionValue < bestSolutionValue) {
            bestSolutionValue = localStat1.bestSolutionValue;
            for (int j = 0; j < 100; j++) {
                bestSolution[j] = localStat1.bestSolution[j];
            }
        }
    }

    for (int i = 0; i < 1000; i++) {
        //calculating similarity to the best solution using number of common nodes
        float commonNodes = 0;
        for (int j = 0; j <100; j++) {
            for (int k = 0; k < 100; k++) {
                if (randomLocalOptima[i][j] == bestSolution[k]) {
                    commonNodes+=1;
                }
            }
        }
        result[i][0] = commonNodes/100;
        //calculating similarity to the best solution using number of common edges
        float commonEdges = 0;
        for (int j = 0; j <100; j++) {
            for (int k = 0; k < 100; k++) {
                if (randomLocalOptima[i][j] == bestSolution[k] && randomLocalOptima[i][(j+1)%100] == bestSolution[(k+1)%100]) {
                    commonEdges++;
                }
                if (randomLocalOptima[i][j] == bestSolution[(k+1)%100] && randomLocalOptima[i][(j+1)%100] == bestSolution[k]) {
                    commonEdges+=1;
                }
            }
        }
        result[i][1] = commonEdges/100;
        //calculating similarity to the average solution using number of common nodes
        float commonNodes2 = 0;
        for (int l = 0; l < 1000; l++) {
            ...
            for (int j = 0; j <100; j++) {
                for (int k = 0; k < 100; k++) {
                    if (randomLocalOptima[i][j] == randomLocalOptima[l][k]) {
                        commonNodes2+=1;
                    }
                }
            }
        }
        result[i][2] = commonNodes2/99900;
        //calculating similarity to the average solution using number of common edges
        float commonEdges2 = 0;
        for (int l = 0; l < 1000; l++) {
            if (l==i) {
                continue;
            }
            for (int j = 0; j <100; j++) {
                for (int k = 0; k < 100; k++) {
                    if (randomLocalOptima[i][j] == randomLocalOptima[l][k] && randomLocalOptima[i][(j+1)%100] == randomLocalOptima[l][(k+1)%100]) {
                    commonEdges2++;
                }
                if (randomLocalOptima[i][j] == randomLocalOptima[l][(k+1)%100] && randomLocalOptima[i][(j+1)%100] == randomLocalOptima[l][k]) {
                    commonEdges2+=1;
                }
                }
            }
        }
        result[i][3] = commonEdges2/99900;
    }

    /*
    ofstream outputFile("localOptimaA.txt");

    if (outputFile.is_open()) {
        for (int i = 0; i < 1000; i++) {
            int distanceValue = 0;
            for (int j = 0; j < 100; j++) {
                outputFile << randomLocalOptima[i][j] << ",";
                distanceValue += distances[randomLocalOptima[i][j]][randomLocalOptima[i][(j+1)%100]];
            }
            outputFile << distanceValue;
            outputFile << "\n";
        }
        outputFile.close();
    ...
    */
    ...
    ofstream outputFile("localOptimaD.txt");

    if (outputFile.is_open()) {
        for (int i = 0; i < 1000; i++) {
            outputFile << result[i][0] << "," << result[i][1] << "," << result[i][2] << "," << result[i][3] << "," << result[i][4] << "\n";
        }
        outputFile.close();
        cout << "Best Solution values written to best_solution.txt" << endl;
    } else {
